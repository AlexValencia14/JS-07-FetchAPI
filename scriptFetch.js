...
Retomando las APIs...
    ...
    - APIs externas (Google maps, Fakestore API, API Paypal, etc.): Son todas aquellas que tenemos que utililzar proveedores externos.
...
Es la capacidad que tiene JS para poder ejecutar funciones que no dependen de otras. Esto nos ayuda a ejecutar ciertos bloques de codigo sin tener que esperar a que termine su ejecución, para ejecutar otras lineas de codigo. (dejar la carne en el asador mientras preparo otras cosas en el ejemplo de la carne asada).
...
function dosAsync() {
    console.log("Uno");
    setTimeout(function () {//setTimeout para ejecutar esta funcion despues de 3 segundos
        console.log("Dos");
    }, 3000);
}
...
Para controlar la sincronia en JS, podemos usar algunos de estos mecanismos:
    ...
    - callback: la forma más basica de manejar la sincronia. Se le conoce como (llamada de vuelta), basicamente es pasar una funcion como parametro de otra funcion, y se ejecutan una vez que se cumpla la condicion esperada.

    //Metodo .map de los arrays

    - promesas: son objetos que representan un valor al momento de conectar al servidor. Como su nombre lo indica, una promesa es algo que no sabemos si se va a cumplir o no, pero al menos necesitamos saber que hacemos si se cumple o si no se cumple. La ventaja que tienen las proomesas, es que no se anidan, en una sola función podemos manejar ambas situaciones.

    Las promesas tienen 3 estados posibles:

        - pending: estado inicial, cuando se crea la promesa. Aqui aun hay resultados.
...
conexion
    ...
    //uso un metodo catch para manejar el error (lo relleno con un error para que la caja no regrese vacia)
    .catch(function (error) {
        console.log("Error", error);
    })

let repuestaServidor = "Felipe de Jesus Maqueda Gonzalez, 31, 2"; //texto plano
...
fetch('https://fakestoreapi.com/products',{ //endpoint
            ...
            body:JSON.stringify( //instruccion para serializar el cuerpo de mi solicitud (para la interpretacion del servidor)
                {
                    title: 'Sopa maruchan de habanero',
                    price: 15.5,
                    description: 'Deleciosa sopa maruchan de habanero',
